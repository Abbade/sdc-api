...
export class CreatePlantsLoteController {
  async handle(request: Request, response: Response) {
    const { id_lote, id_location,id_recipiente,aclimatationDate, qtPlant, obs, scheduled, id_user_atribution} = request.body;
    ...
    const result = await createPlantsLoteUseCase.execute({
      id_lote, aclimatationDate, qtPlant, obs, id_user_create, id_location, id_recipiente, id_user_atribution, scheduled
    });
